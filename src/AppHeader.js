...
function AppHeader(props) {
  ...
  const showAddDataset =
    ...
    user.importer &&
    !loadingApp.loading &&
    serverInfo.capabilities.has(SERVER_CAPABILITY_ADD_DATASET);
  const showEditDataset =
    dataset !== null &&
    dataset.owner &&
    !loadingApp.loading &&
    serverInfo.capabilities.has(SERVER_CAPABILITY_EDIT_DATASET);
  const showDeleteDataset =
    dataset !== null &&
    dataset.owner &&
    !loadingApp.loading &&
    serverInfo.capabilities.has(SERVER_CAPABILITY_DELETE_DATASET);
  const showMoreMenu =
    (showAddDataset ||
      showEditDataset ||
      showDeleteDataset ||
      ...
      >
                    ...
                    <div key={item.fieldName}>
                      ...
                      {item.fieldName === 'description' && (
                        <ReactMarkdown
                          ...
                          children={dataset[item.fieldName]}
                        />
                      )}
                    </div>
                  );
        ...
        <Toolbar variant="dense" style={{paddingLeft: 0}}>
          ...
          <div style={{display: 'flex', marginLeft: 'auto'}}>
            ...
            {!loadingApp.loading && !isSignedOut && (
              <DatasetSelector onChange={onDataset} />
            )}
            {showMoreMenu && (
              <Tooltip title={'More'}>
                <IconButton
            ...
            {showMoreMenu && (
              ...
              >
                {showAddDataset && (
                  <MenuItem onClick={onImportDataset}>New Dataset</MenuItem>
                )}

                {showEditDataset && (
                  <MenuItem onClick={onSettings}>Edit Dataset</MenuItem>
                )}
                {showDeleteDataset && (
                  <MenuItem onClick={onDelete}>Delete Dataset</MenuItem>
                )}
                {(showAddDataset || showEditDataset || showDeleteDataset) &&
                  dataset != null && <Divider />}
                {dataset != null && (
